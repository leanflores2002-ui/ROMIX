...
if(!/^https:\/\//.test(base || '')) throw new Error('Provide HTTPS deployment origin');
async function main(){
  ...
  for(const p of products) {if(chosen.size>=10)break;add(p);}
  const results=[];
  for(const [slug,p] of chosen){
    const url=`${base.replace(/\/$/,'')}/share/${slug}/`;
    const page=await fetch(url,{redirect:'manual',headers:{'user-agent':'facebookexternalhit/1.1'},signal:AbortSignal.timeout(30000)});
    const html=await page.text();const dom=new JSDOM(html);
    ...
    const image=await fetch(imageUrl,{redirect:'manual',headers:{'user-agent':'facebookexternalhit/1.1'},signal:AbortSignal.timeout(30000)});
